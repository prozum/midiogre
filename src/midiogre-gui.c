...
GlobalWidgets *g_wgs_init(GtkWidget *window)
{
    ...
    gtk_box_pack_start(g_wgs->win_box, frame, FALSE, FALSE, 10);
    g_wgs->search_box = GTK_BOX(gtk_box_new(GTK_ORIENTATION_VERTICAL, 10));
    gtk_container_set_border_width(GTK_CONTAINER(g_wgs->search_box), 20);
    ...
    gtk_box_pack_start(g_wgs->search_box, GTK_WIDGET(box), FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(box, GTK_WIDGET(g_wgs->title_entry), FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(box, GTK_WIDGET(g_wgs->artist_entry), FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(box, GTK_WIDGET(g_wgs->album_entry), FALSE, FALSE, 0);
    ...
    gtk_container_set_border_width(GTK_CONTAINER(g_wgs->instr_box), 20);
    ...
    /* Add song boxes */
    g_wgs->songbox[0] = songbox_new(g_wgs->win_box, "<span size=\"large\">Best Match</span>", (GtkListBoxSortFunc) song_row_sort);
    g_wgs->songbox[1] = songbox_new(g_wgs->win_box, "<span size=\"large\">Random</span>", (GtkListBoxSortFunc)song_row_sort);
    g_wgs->songbox[2] = songbox_new(g_wgs->win_box, "<span size=\"large\">Newest</span>", (GtkListBoxSortFunc)song_row_sort);
