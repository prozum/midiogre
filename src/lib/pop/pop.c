...
double song_score(unsigned int plays, unsigned int upload_time, uint8_t fingerprint)
{
...
int song_compare(const void *s1, const void *s2)
{
    ...
    song1_score = song_score(song1->plays,song1->time_added, finger_prn_arr_cmp(song1->fingerprint, song1->fingerprint));
    song2_score = song_score(song2->plays,song2->time_added, finger_prn_arr_cmp(song2->fingerprint, song2->fingerprint));
